...
__global__ void DrawY(uint8_t *frame, const double iteration) {
    ...
    if (y < H and x < W) {
        ...
        double xi = 0.0;
        double yi = 0.0;
        double midx = double(W)/2 + 2.0*iteration;
        double midy = double(H)/2;
        double scaleFactor = 0.5*(1 + iteration/double(NFRAME)/0.5);
        double scalex = double(W)*scaleFactor*scaleFactor;
        double scaley = double(H)*scaleFactor*scaleFactor;
        double x0 = (double(x) - midx)/scalex;
        double y0 = (double(y) - midy)/scaley;
        double xtemp, lengthsq = 0.0;
        while(c < iteration/4.0 and lengthsq < 10.0) {
...
void Lab1VideoGenerator::Generate(uint8_t *yuv) {
    DrawY<<<dim3((W-1)/16+1,(H-1)/12+1), dim3(16,12)>>>(yuv, (double)impl->t);
